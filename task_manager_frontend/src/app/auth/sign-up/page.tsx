...
import { apiSignUp } from "@/lib/backend";
      ...
      <div className="retro-card p-5 bg-white">
        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            setError(null);
            try {
              const session = await apiSignUp(email.trim(), password, name.trim() || undefined);
              signIn(session);
              router.push("/tasks");
            } catch (err) {
              const msg = err instanceof ApiError ? err.message : "Failed to sign up.";
              setError(msg);
            } finally {
              setBusy(false);
            }
          ...
          {error ? (
            <div className="retro-card-soft p-3 border-red-600">
              ...
              <div className="text-xs text-gray-600 mt-1">
                If backend register route is not implemented yet, you will see errors.
              </div>
            </div>
          ) : null}
